function [Omega,S,loglik,iter] = identKron8X(X,p,nl,ng,e,tol,verb,flag_init)
% Optimization sequence: Sigma, Gamma,  Lambda 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 function [Lambda_new, Gamma_new]=reweight(S,Lambda_old,Gamma_old,T,e,nl,ng)
    ...
    Gamma_new=Gamma_old;
    lam = reshape(triu(Lambda_old),nl^2,1);
    [lams is]=sort(-lam);
    ...
    for j=1:ng
        for k=j:ng
            ...
            for l=1:nl*(nl+1)
                obj(l,1)=obj_hyperGamma(S,T,Lambda_old,Gamma_new,e,sol(l),j,k);
            end
            if  sum(sum(Lambda_old<1e-8))>0 
                sol(nl*(nl+1)+1,1)=0;
                obj(nl*(nl+1)+1,1)=obj_hyperGamma(S,T,Lambda_old,Gamma_new,e,0,j,k);
            end
            [val pos]=min(obj);
    ...
    gam = reshape(triu(Gamma_new),ng^2,1);
    [gams is]=sort(-gam);
    ...
    for j=1:nl
        for k=j:nl
            ...
            for l=1:ng*(ng+1)
                obj(l,1)=obj_hyperLambda(S,T,Lambda_new,Gamma_new,e,sol(l),j,k);
            end
            if sum(sum(Gamma_new<1e-8))>0
                sol(ng*(ng+1)+1,1)=0;
                obj(ng*(ng+1)+1,1)=obj_hyperGamma(S,T,Lambda_new,Gamma_new,e,0,j,k);
            end
            [val pos]=min(obj);
